...
- Located in the `data/page_level_metrics/` subfolder
- Ideally consolidated into a single file such as `page_level_visual_metrics.csv`
- Each row represents a single page from a sustainability report
- Details include: `year` of publication, originating company `code`, `page_number`, values for the 18 specific visual sub-elements, and calculated page-specific Feature-correlation Index (`NFIVI_FC`) and Information Entropy Index (`NFIVI_EI`)
...
- Available in the `data/report_level_metrics.csv` file
- Each record corresponds to an entire sustainability report (PDF)
- Provides aggregated values for the 18 core visual indicators, `year`, company `code`, and report-level Information Entropy Index (`NFIVI_EI`)

**3. Report-Level Metrics (Feature-correlation Index via Mahalanobis Distance)**
- Provided in `data/report_level_metrics.csv`
- Contains one record per report
- Includes `year`, company `code`, and `NFIVI_FC`
...
- `compute_mahalanobis_fc_page.py` - Details the calculation of `NFIVI_FC` at the page level
- `compute_mahalanobis_fc_report.py` - Covers the report-level `NFIVI_FC` using Mahalanobis distance
- `compute_information_entropy_indices.py` - Outlines the derivation of the `NFIVI_EI` for both page and report levels
